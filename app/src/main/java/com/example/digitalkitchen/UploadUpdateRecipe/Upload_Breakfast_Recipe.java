...
public class Upload_Breakfast_Recipe extends AppCompatActivity {
    ...
    private static final String TAG = "Upload_Breakfast_Recipe";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        home.setOnClickListener(v -> startActivity(new Intent(Upload_Breakfast_Recipe.this, CategoryRecipeActivity.class)
        ...
        profile.setOnClickListener(v -> startActivity(new Intent(Upload_Breakfast_Recipe.this, myProfilePage.class)
                .putExtra("name", nameUser)
                .putExtra("email", emailUser)
                .putExtra("username", usernameUser)
                .putExtra("password", passwordUser)
            ...
            storageReference.putFile(uri).addOnSuccessListener(taskSnapshot -> {
                ...
                taskSnapshot.getStorage().getDownloadUrl().addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        ...
                        progressDialog.dismiss();
                        Toast.makeText(Upload_Breakfast_Recipe.this, "העלאת התמונה נכשלה", Toast.LENGTH_SHORT).show();
    ...
    public void uploadRecipe(AlertDialog progressDialog) {
        ...
        FirebaseDatabase.getInstance(Utils.DATABASE_URL).getReference("users")
                .child(usernameUser).child("RecipeStarter").child(myCurrentDateTime).setValue(foodData)
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        Utils.playSound(this, R.raw.success_sound);
                        Toast.makeText(Upload_Breakfast_Recipe.this, "המתכון הועלה בהצלחה", Toast.LENGTH_SHORT).show();
                        Log.d(TAG, "uploadRecipe: Recipe uploaded successfully");
                        finish();
                    } else {
                        Toast.makeText(Upload_Breakfast_Recipe.this, "שמירת המתכון נכשלה", Toast.LENGTH_SHORT).show();
